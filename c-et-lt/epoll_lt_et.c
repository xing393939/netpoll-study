...
/* 最大缓存区大小，redis-cli发送ping命令是14个字节，所以一次read读不完，需要两次 */
...
int main(int argc, const char *argv[]) {
    ...
    sockfd = create_socket("127.0.0.1", 6378);
    struct epoll_event events[MAX_EPOLL_EVENTS];

    if ((epollfd = epoll_create1(0)) == -1)
        err_exit("epoll_create1() error");

    /* 1.sockfd：非阻塞的LT模式，监听fd没有必要设置为阻塞的 */
    addfd_to_epoll(epollfd, sockfd, EPOLL_LT, FD_NONBLOCK);

    /* sockfd：非阻塞的ET模式 */
    // 2.addfd_to_epoll(epollfd, sockfd, EPOLL_ET, FD_NONBLOCK);

    while (1) {
        // epoll_wait的timeout=-1表示无限阻塞
